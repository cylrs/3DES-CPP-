...
template<int maxsize> SequenceD<maxsize>
...
/*
template<int maxsize>
 SequenceD SequenceD<maxsize>::get()
{
    return seqq;
}

template<int maxsize>
 void SequenceD<maxsize>::pop(){
     seqq.pop();
 }
...
template<int maxsize =64>
 ostream& write(SequenceD<64> s,std::ostream& outStr){
    /*Sequence left = s.left();
    Sequence right = s.right();
    string txt = left.afficheT() + right.afficheT();
   return outStr.write((txt).c_str(),s.size());
      //return outStr;
   for (int i = 0; i < s.size(); i++) outStr<<s(i);
   outStr<<endl;
   return outStr;
...
template<int Size>
ostream& write(sequenceD<Size> seq , ostream& os )
{
	if (Size != 64)
		return os;
	os << endl;
